...
def infer_mapper_config(state_dict: Dict[str, torch.Tensor], meta: Dict[str, object]) -> Tuple[str, Optional[Dict[str, object]]]:
    ...
    if mapper_type == 'mrc':
        params = meta.get('params', {}) if isinstance(meta, dict) else {}
        levels_per_axis = int(mapper_cfg.get('levels_per_axis', params.get('mrc_levels_per_axis', 4)))
        mapper_kwargs = {
            'levels_per_axis': levels_per_axis,
            'init_bounds': mapper_cfg.get('init_bounds', params.get('mrc_init_bounds', '')),
